...
import { HeroDecisionPanelV2 } from './HeroDecisionPanelV2';
import { TrustChangeBand } from './TrustChangeBand';
import { EvidenceMatrixV2 } from './EvidenceMatrixV2';
...
export const SignalDashboardV2 = () => {
    ...
    if (!isLoaded) {
        return (
            <CockpitPageShell theme={theme}>
                <AppNav active="signal" tone={theme} isV2={true} />
    ...
    return (
        <CockpitPageShell theme={theme}>
            ...
            <div className={`cockpit-theme-scope mx-auto max-w-7xl px-4 pb-20 pt-6 sm:px-6 lg:px-8 transition-all duration-300 ${themeClasses.pageText}`}>
                <DashboardHeaderV2
            ...
            {!signalData && !loading && (
                <section className={`rounded-3xl border p-8 shadow-[0_20px_60px_rgba(2,6,23,0.16)] transition-all duration-350 ${theme === 'light' ? 'border-rose-300 bg-rose-50 text-rose-950' : 'border-rose-500/25 bg-rose-500/10 text-slate-100'}`}>
            ...
            {signalData && (
                <div className="space-y-6">
                    ...
                    <EvidenceMatrixV2 signal={signalData} market={config.market} theme={theme} />
...
const CockpitPageShell = ({ theme, children }: { theme: CockpitTheme; children: React.ReactNode }) => {
    ...
    const mainClass = isLight
        ? 'cockpit-page relative min-h-screen overflow-x-hidden bg-[#f8fafc] text-slate-950 selection:bg-sky-200 selection:text-slate-950 transition-colors duration-300'
        : 'cockpit-page relative min-h-screen overflow-x-hidden bg-[#0b1118] text-slate-100 selection:bg-emerald-300 selection:text-slate-950 transition-colors duration-300';
    const orbClass = isLight
        ? 'absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.11),_transparent_28%),radial-gradient(circle_at_80%_10%,_rgba(16,185,129,0.1),_transparent_20%)] transition-opacity duration-300'
        : 'absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.12),_transparent_24%),radial-gradient(circle_at_80%_10%,_rgba(52,211,153,0.1),_transparent_18%)] transition-opacity duration-300';
    const gridClass = isLight
        ? 'absolute inset-0 bg-[linear-gradient(rgba(148,163,184,0.08)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.08)_1px,transparent_1px)] bg-[size:44px_44px] opacity-45 transition-opacity duration-300'
        : 'absolute inset-0 bg-[linear-gradient(rgba(148,163,184,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.045)_1px,transparent_1px)] bg-[size:44px_44px] opacity-55 transition-opacity duration-300';
