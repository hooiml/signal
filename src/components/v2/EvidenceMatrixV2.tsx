...
export const EvidenceMatrixV2 = ({ signal, market, theme }: EvidenceMatrixProps) => {
    ...
    return (
        <section className={`rounded-3xl border p-5 transition-all duration-300 ${themeClasses.panelStrong}`}>
            ...
            {disagreementNote && (
                <div className={`mt-4 rounded-2xl border px-4 py-3 text-sm leading-6 transition-all ${theme === 'light' ? 'border-amber-300 bg-amber-50 text-amber-800' : 'border-amber-500/25 bg-amber-500/10 text-amber-100'}`}>
                ...
                <table className={`min-w-full divide-y text-left text-sm ${themeClasses.divider} ${themeClasses.textSecondary}`}>
                    ...
                    <tbody className={`divide-y ${themeClasses.divider}`}>
                        {rows.map((row) => (
                            <tr
                                key={row.name}
                                className={`align-top transition-colors duration-250 hover:bg-slate-500/5 ${row.supportState === 'challenges'
                            ...
                            >
                                ...
                                <td className={`px-4 py-4 font-mono ${themeClasses.textSecondary}`}>
                                    ...
                                    <div className={`mt-2 h-1.5 overflow-hidden rounded-full ${themeClasses.railBackground}`}>
                                        <div
                                            className={`h-full rounded-full bg-gradient-to-r ${row.supportState === 'challenges'
            ...
            <div className="mt-5 space-y-3 lg:hidden">
                {rows.map((row) => (
                    <article key={row.name} className={`rounded-2xl border p-4 transition-all duration-200 hover:scale-[1.01] ${row.supportState === 'challenges'
...
const SummaryChip = ({ label, value, themeClasses }: { label: string; value: string; themeClasses: ReturnType<typeof getThemeClasses> }) => (
    <div className={`rounded-xl border px-3 py-2 transition-all ${themeClasses.statChip}`}>
