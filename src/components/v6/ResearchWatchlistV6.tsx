    ...
    return (
        <aside className={'min-w-0 border-b pb-4 min-[700px]:w-[220px] min-[700px]:shrink-0 min-[700px]:border-b-0 min-[700px]:border-r min-[700px]:pb-0 min-[700px]:pr-4 ' + themeClasses.divider}>
            <div className={'mb-4 flex items-center justify-between gap-3 border-b px-1 pb-3 ' + themeClasses.divider}>
                ...
                <button type="button" onClick={() => setShowAdd((current) => !current)} className={'min-h-10 rounded px-3 text-[11px] font-bold ' + themeClasses.positive}>{showAdd ? 'Close' : '+ Add'}</button>
            </div>
            {showAdd ? (
            ...
            {items.length > 0 ? (
                <div className="research-scrollbar flex gap-2 overflow-x-auto pb-2 min-[700px]:flex-col min-[700px]:overflow-visible min-[700px]:pb-0">
                    {items.map((item) => {
