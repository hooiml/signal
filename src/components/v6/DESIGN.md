...
- The story trust strip is the single compact orientation summary and keeps the prior-score comparison attached to the current composite score. A concise Historical forward outcomes section follows the story and describes the one-week and one-month results after comparable score zones without presenting them as predictions. A visible Why this score section then keeps score history beside contribution-ranked weighted evidence with inline prior-driver comparisons.
...
- On narrow screens the reading order remains headline and trust strip, chapters, historical forward outcomes, Why this score, full calibration, scenarios, context, alerts, terms, and details. The one-week and one-month summaries stack, score and driver comparisons stack beneath their current values, and no narrative panel may require horizontal scrolling.
