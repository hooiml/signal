    ...
    return (
        <section data-testid="expectation-reality" className="mt-3 rounded-lg border border-zinc-700/40 p-3" aria-label={`Expectation versus reality for ${ticker}`}>
            <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <button type="button" onClick={startNew} className="min-h-11 rounded-lg border border-zinc-700 px-3 py-2 text-xs font-semibold hover:border-emerald-500 focus-visible:outline-2 focus-visible:outline-emerald-500">New event</button>
            </div>

            {events.length > 0 && (
                <div className="mt-3 flex gap-2 overflow-x-auto pb-1" aria-label="Saved expectation events">
                    {events.slice(0, 6).map((event) => (
                        <button key={event.id} type="button" onClick={() => setDraft(event)} aria-pressed={draft.id === event.id} className={`min-h-11 shrink-0 rounded-lg border px-3 py-2 text-left text-xs focus-visible:outline-2 focus-visible:outline-emerald-500 ${draft.id === event.id ? 'border-emerald-500' : 'border-zinc-700/50'}`}>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
                <label className="grid gap-1 text-xs"><span className="font-semibold">Event title</span><input aria-label="Event title" value={draft.title} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value, updatedAt: new Date().toISOString() }))} className="min-h-11 rounded-lg border border-zinc-700/50 bg-transparent px-3 focus-visible:outline-2 focus-visible:outline-emerald-500" /></label>
                <label className="grid gap-1 text-xs"><span className="font-semibold">Event date</span><input aria-label="Event date" type="date" value={draft.eventDate} onChange={(event) => setDraft((current) => ({ ...current, eventDate: event.target.value, updatedAt: new Date().toISOString() }))} className="min-h-11 rounded-lg border border-zinc-700/50 bg-transparent px-3 focus-visible:outline-2 focus-visible:outline-emerald-500" /></label>
                <label className="grid gap-1 text-xs"><span className="font-semibold">Stage</span><select aria-label="Event stage" value={draft.status} onChange={(event) => setDraft((current) => ({ ...current, status: event.target.value as ResearchExpectationEvent['status'], updatedAt: new Date().toISOString() }))} className="min-h-11 rounded-lg border border-zinc-700/50 bg-transparent px-3 focus-visible:outline-2 focus-visible:outline-emerald-500"><option value="pre-event">Pre-event</option><option value="reported">Reported</option><option value="reviewed">Reviewed</option></select></label>
            </div>

            <div className="mt-3 sm:overflow-x-auto">
                <table className="w-full text-left text-xs sm:min-w-[560px]">
                    <thead className="hidden text-zinc-500 sm:table-header-group"><tr><th className="pb-2 pr-3">Metric</th><th className="pb-2 pr-3">Expected</th><th className="pb-2 pr-3">Actual</th><th className="pb-2">Outcome</th></tr></thead>
                    <tbody data-testid="expectation-metric-cards" className="grid gap-3 sm:table-row-group">{comparison.metrics.map((metric) => (
                        <tr key={metric.id} className="grid grid-cols-2 gap-3 rounded-lg border border-zinc-700/40 p-3 sm:table-row sm:rounded-none sm:border-x-0 sm:border-b-0 sm:p-0">
                            <th className="col-span-2 font-semibold sm:table-cell sm:py-2 sm:pr-3">{metric.label}{metric.unit ? ` (${metric.unit})` : ''}</th>
                            <td className="min-w-0 sm:table-cell sm:py-2 sm:pr-3"><label className="grid gap-1"><span className="font-semibold sm:hidden">Expected</span><input aria-label={`${metric.label} expected`} inputMode="decimal" value={numberValue(metric.expected)} onChange={(event) => updateMetric(metric.id, 'expected', event.target.value)} className="min-h-11 w-full min-w-0 rounded-md border border-zinc-700/50 bg-transparent px-2 focus-visible:outline-2 focus-visible:outline-emerald-500 sm:w-28" /></label></td>
                            <td className="min-w-0 sm:table-cell sm:py-2 sm:pr-3"><label className="grid gap-1"><span className="font-semibold sm:hidden">Actual</span><input aria-label={`${metric.label} actual`} inputMode="decimal" value={numberValue(metric.actual)} onChange={(event) => updateMetric(metric.id, 'actual', event.target.value)} className="min-h-11 w-full min-w-0 rounded-md border border-zinc-700/50 bg-transparent px-2 focus-visible:outline-2 focus-visible:outline-emerald-500 sm:w-28" /></label></td>
                            <td className="col-span-2 sm:table-cell sm:py-2"><span className="mr-2 font-semibold sm:hidden">Outcome</span><strong>{outcomeLabel(metric.outcome)}</strong>{metric.variancePercent !== null && <span className="ml-2 text-zinc-500">{metric.variancePercent >= 0 ? '+' : ''}{metric.variancePercent.toFixed(1)}%</span>}</td>
                        </tr>
                    ))}</tbody>
                </table>
            </div>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
                <label className="grid gap-1 text-xs"><span className="font-semibold">What was expected?</span><textarea aria-label="Expected narrative" rows={3} value={draft.expectedNarrative} onChange={(event) => setDraft((current) => ({ ...current, expectedNarrative: event.target.value, updatedAt: new Date().toISOString() }))} className="rounded-lg border border-zinc-700/50 bg-transparent p-3 focus-visible:outline-2 focus-visible:outline-emerald-500" placeholder="What did investors appear to require? Which metric mattered most?" /></label>
                <label className="grid gap-1 text-xs"><span className="font-semibold">What actually happened?</span><textarea aria-label="Actual narrative" rows={3} value={draft.actualNarrative} onChange={(event) => setDraft((current) => ({ ...current, actualNarrative: event.target.value, updatedAt: new Date().toISOString() }))} className="rounded-lg border border-zinc-700/50 bg-transparent p-3 focus-visible:outline-2 focus-visible:outline-emerald-500" placeholder="Record the result before explaining it with hindsight." /></label>
            </div>
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-[180px_minmax(0,1fr)]">
                ...
                <label className="grid gap-1 text-xs"><span className="font-semibold">Interpretation</span><input aria-label="Expectation interpretation" value={draft.interpretation} onChange={(event) => setDraft((current) => ({ ...current, interpretation: event.target.value, updatedAt: new Date().toISOString() }))} className="min-h-11 rounded-lg border border-zinc-700/50 bg-transparent px-3 focus-visible:outline-2 focus-visible:outline-emerald-500" placeholder="Why might the reaction differ from the headline result?" /></label>
            </div>
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-zinc-700/30 p-3">
                ...
                <button type="button" onClick={() => void save()} disabled={saving} className="min-h-11 rounded-lg border border-emerald-600 px-4 py-2 text-xs font-semibold disabled:opacity-50 focus-visible:outline-2 focus-visible:outline-emerald-500">{saving ? 'Saving…' : 'Save review'}</button>
            </div>
