...
import { ConnectedHistory, defaultHistory } from './MarketV8ConnectedHistory';
import { MarketCoverageNotices } from './MarketV8Coverage';
import { ConnectedIndicator, ConnectedPanels, emptyScenario } from './MarketV8ConnectedPanels';
...
export function MarketV8Connected() {
    ...
    const priorView = useRef<{ tab: InvestigationTab; selected: string | null; point: Point | null; scroll: number }>({ tab: 'What changed', selected: null, point: null, scroll: 0 });
    ...
    useEffect(() => {
        ...
        const sync = () => {
            if (media.matches) { if (!modal.current?.open) modal.current?.showModal(); document.body.style.overflow = 'hidden'; }
                ...
                const wasModal = modal.current?.open;
                modal.current?.close(); document.body.style.overflow = previous;
                if (wasModal) (trigger.current?.isConnected ? trigger.current : rail.current)?.focus({ preventScroll: true });
            }
        };
        ...
        <div className={styles.canvas}>
            ...
            <main id="market-content" tabIndex={-1} className={styles.workspace}>
                <div className={styles.mainColumn}>
                    ...
                    {selected && <div className={styles.inspectorAccess}><span role="status">{getIndicatorDisplayName(selected)} detail updated.</span><button onClick={() => rail.current?.focus({ preventScroll: true })}>Move to indicator detail</button></div>}
                    ...
                    {data && hasScore && <>
                        {historical && <div className={styles.historyBanner}><span>{fullDate(historical.date)} · {archived?.summary.origin ?? data.metadata.score_history?.find(point=>point.date===historical.date)?.origin ?? 'Origin unavailable'}</span><button onClick={returnCurrent}>Return to current ↗</button></div>}
                ...
                <aside className={`${styles.rail} ${selected?styles.railSelected:''}`} ref={rail} tabIndex={-1} id="indicator-inspector" aria-label={selected?'Selected indicator':'Reading context'}>{inspector ?? <><span className={styles.eyebrow}>The wider reading</span><h2>{historical?'Dated evidence only.':'Evidence and its limits.'}</h2>{data&&hasScore&&!historical?<>
...
function ArchiveSpark({ groups, loading, signal }: { groups: Point[][]; loading: boolean; signal?: MarketSignal['tier'] }) {
    ...
    const color = signal === 'buy' || signal === 'strong-buy' ? '#006344' : signal === 'sell' || signal === 'strong-sell' ? '#97263f' : '#666666';
    return <svg className={styles.sparkline} style={{ color }} viewBox="0 0 160 44" aria-hidden="true" data-testid="archive-spark">{groups.map((group,index)=><g key={index}><path d={group.map((point,i)=>`${i?'L':'M'}${x(point)},${y(point)}`).join(' ')} fill="none" stroke="currentColor" strokeWidth="2"/>{group.map(point=><circle key={point.date} cx={x(point)} cy={y(point)} r="2" fill="currentColor"/>)}</g>)}</svg>;
}
