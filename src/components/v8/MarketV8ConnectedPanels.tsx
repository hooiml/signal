...
function ScenarioPanel({ signal }: { signal: MarketSignal }) {
    ...
    return <>
        ...
        <div className={styles.scenarioGrid}><div><label className={styles.field}>Input to explore<select aria-label="Scenario input" value={effectiveKey} onChange={(event) => setSelectedKey(event.target.value)}>{result.drivers.map((driver) => <option key={driver.key} value={driver.key}>{driver.name}</option>)}</select></label>{selected ? <><label className={styles.sliderLabel} htmlFor="connected-scenario-score">Hypothetical normalized score <b>{overrides[effectiveKey] ?? selected.baseScore.toFixed(2)}</b></label><input id="connected-scenario-score" type="range" min="0" max="100" step="1" value={overrides[effectiveKey] ?? selected.baseScore} onChange={(event) => setOverrides((current) => ({ ...current, [effectiveKey]: Number(event.target.value) }))} /><div className={styles.scaleLabels}><span>0</span><span>50 · neutral</span><span>100</span></div></> : null}<button className={styles.secondaryButton} onClick={() => setOverrides({})}>Reset scenario</button></div><div className={styles.simulated} aria-live="polite"><span>Simulated score</span><strong>{result.simulatedScore}<small>/100</small></strong><b>{signed(result.scoreDelta, 0)} vs current {signal.composite_score}</b><p>{result.simulatedTier} · hypothetical only</p></div></div>
