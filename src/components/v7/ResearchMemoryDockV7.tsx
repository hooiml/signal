...
export const ResearchMemoryDockV7 = () => {
    ...
    if (status === 'loading') {
        return <section data-testid="research-memory-dock" aria-busy="true" className="mx-auto mb-4 w-full max-w-[1440px] px-3 sm:px-5"><div className="rounded-xl border border-zinc-700/40 bg-zinc-900/20 p-4 text-sm text-zinc-500">Building decision memory for {ticker}…</div></section>;
    }

    if (!model || !record) {
        return <section data-testid="research-memory-dock" className="mx-auto mb-4 w-full max-w-[1440px] px-3 sm:px-5"><div className="rounded-xl border border-zinc-700/40 bg-zinc-900/20 p-4"><strong className="text-sm">Decision memory · {ticker}</strong><p className="mt-1 text-xs text-zinc-500">{message ?? 'No memory is available yet.'}</p></div></section>;
    }
    ...
    return (
        <section data-testid="research-memory-dock" className="mx-auto mb-4 w-full max-w-[1440px] px-3 sm:px-5" aria-label={`Decision memory for ${ticker}`}>
            <div className="rounded-xl border border-zinc-700/40 bg-zinc-950/30 p-4 shadow-sm">
