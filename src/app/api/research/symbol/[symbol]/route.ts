...
export const GET = async (request: Request, context: RouteContext): Promise<NextResponse> => {
    ...
    try {
        ...
        return NextResponse.json({ success: true, data: await getResearchSnapshot(symbol, researchMarket) });
    } catch (error) {
        return NextResponse.json({ success: false, error: error instanceof Error ? error.message : 'Free data sources unavailable.' }, { status: 502 });
    }
};
