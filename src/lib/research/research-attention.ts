...
import { rankResearchAttentionItems } from './research-attention-rank';
...
const stateItemsForRecord = async (record: ResearchRecord, now: Date): Promise<ResearchInboxItem[]> => {
    ...
    if (latestEvent) {
        ...
        if (until <= 0 && pendingActuals) items.push({
            id: `${symbol}-attention-expectation-${latestEvent.id}-actuals`, symbol, kind: 'expectation', urgency: 'action', title: 'Complete expectation vs reality',
            ...
            proximity: until === 0 ? 'Event is today' : `${Math.abs(until)} day${Math.abs(until) === 1 ? '' : 's'} after event`, source: 'Expectation journal', eventDate: latestEvent.eventDate, structuredTriggerRuleId: null,
        });
        else if (until > 0 && until <= 14 && pendingExpectations) items.push({
            id: `${symbol}-attention-expectation-${latestEvent.id}-pre`, symbol, kind: 'expectation', urgency: 'upcoming', title: 'Capture pre-event expectations',
    ...
    if (valuationPlan && valuationPlan.currentEps === null) items.push({
        id: `${symbol}-attention-valuation-input`, symbol, kind: 'valuation', urgency: 'action', title: 'Complete valuation evidence input',
    ...
    if (latestReview && !calibrations.some((entry) => entry.reviewId === latestReview.id)) {
        ...
        if (ageDays >= 14) items.push({
            id: `${symbol}-attention-calibration-${latestReview.id}`, symbol, kind: 'decision', urgency: 'action', title: 'Review the prior decision process',
...
export const enrichResearchInboxWithAttention = async (base: ResearchInboxResponse, symbols: readonly string[], now = new Date(base.generatedAt)): Promise<ResearchInboxResponse> => {
