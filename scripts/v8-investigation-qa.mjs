...
try{
    ...
    await page.getByRole('button',{name:/^Saved securities/}).click();await page.getByRole('region',{name:'Saved watchlist'}).getByRole('button',{name:/MSFT/}).click();await page.getByRole('article',{name:'MSFT research'}).waitFor();assert.equal(await page.getByRole('region',{name:'Price history',exact:true}).getByRole('button',{name:'1M',exact:true}).getAttribute('aria-pressed'),'true');
