...
The UI withholds outcome statistics until a zone has at least five eligible observations. A directional sample is not labelled established until it has at least 20 eligible outcomes including at least five observed scores; reconstruction-only evidence remains preliminary regardless of its row count. A concise current-zone summary shows one-week and one-month median return, positive-period frequency, sample size, observed-versus-reconstructed provenance, and the existing evidence level. It describes historical outcomes and must not be framed as a probability or prediction. The later Historical calibration section owns the full score-zone comparison, reconstruction coverage, and methodology so the current-zone summary is not repeated there. These are overlapping historical observations without transaction costs, not a backtest or forecast, and the limitation must remain visible wherever the calibration is rendered.
