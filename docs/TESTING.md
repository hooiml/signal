...
Picker smoke checks should restore `workspace=picker`, retain setup until Run picker is activated, and reuse the validated Discovery response without generating another score. Exercise conservative and balanced profiles, every minimum-score and pick-count boundary, loading, provider error/retry, no-match, and result states. Start, reload, inspect, and remove a browser-local paper basket; verify its timestamp, selection rule, entry prices, scores, and current observed moves when a selected symbol remains in the bounded scan. Confirm no Research record mutation, no candidate-specific probability or buy language, local table scrolling, and no document overflow at 1280px, 768px, and 375px.
...
Research workflow queue checks should reject malformed tasks and source labels, migrate older tasks to manual provenance, cap local history at 100, replace by task ID, keep pending tasks ahead of completed work, deduplicate pending connected tasks by ticker/template/source while retaining the earliest due date, and preserve each fixed template's field list. Browser verification should create an overdue task, reload to prove local persistence, complete/reopen/remove tasks, and start at least Earnings update and Valuation refresh. Queue a review from Thesis Changes, Calendar, Alerts, and Market Exposure; confirm the appropriate template, source badge, due date, same-tab update, and duplicate response. The focused editor must show only the template's narrative fields while retaining checklist, thesis strength, valuation, target zone, position plan, decision journal, accepted evidence, cancel, and Save review. Confirm queue actions alone never call a research-record mutation and check document overflow at 1280px, 768px, and 375px.
...
Command-palette and saved-view checks should reject malformed workspace, ticker, tab, filter, and density values; deduplicate names case-insensitively; cap storage at eight views; and fall back to comfortable density. Browser verification should open the palette with Ctrl/Cmd+K from both Market and Research, filter commands, use Arrow keys/Enter/Escape, restore focus, run representative Market configuration, Research workspace, ticker, and saved-view commands, and confirm no research mutation request is emitted by navigation. Save, replace, apply, reload, and remove a named Research view; verify its workspace, query/filter, ticker, detail tab, and density state. Check the modal and compact/comfortable Research layouts for focus visibility and document overflow at 1280px, 768px, and 375px.
